...
export function channelToColor(channel) {
...
export function align8(a) {
    let result = ((a + 15) & 0xfffffff0);
    return result;
}
export function colorToId(color) {
    if (!color) {
        return -1;
    }
    let g = (color >> 8) & 255;
...
export function getNormal3(p1, p2, p3) {
...
export function getNormal2(p1, p2) {
    let dx = p2[0] - p1[0];
    let dy = p2[1] - p1[1];
    let result = vec3.create();
    return vec3.normalize(result, vec3.fromValues(-dy, dx, 0));
